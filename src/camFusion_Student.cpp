...
void show3DObjects(std::vector<BoundingBox> &boundingBoxes, cv::Size worldSize, cv::Size imageSize, bool bWait)
{
    ...
    for(auto it1=boundingBoxes.begin(); it1!=boundingBoxes.end(); ++it1)
    {
        ...
        putText(topviewImg, str1, cv::Point2f(left-50, bottom+50), cv::FONT_ITALIC, 0.75, currColor);
        sprintf(str2, "xmin=%2.2f m, yw=%2.2f m", xwmin, ywmax-ywmin);
        putText(topviewImg, str2, cv::Point2f(left-45, bottom+125), cv::FONT_ITALIC, 0.50, currColor);  
    }
                      ...
                      std::vector<cv::DMatch> kptMatches, double frameRate, double &TTC, cv::Mat *visImg)
{
    ...
    double medianDistRatio = distanceRatios.size() % 2 == 0 ? (distanceRatios[medianIdx - 1] + distanceRatios[medianIdx]) / 2.0 : distanceRatios[medianIdx]; 
    
    // get median ratio to remove outliers
